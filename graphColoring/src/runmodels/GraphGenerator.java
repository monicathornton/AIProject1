...
//		BufferedWriter graphWriter100 = null;
		...
		for (int i = 0; i < size; i++) {
			// Gets the random x and y location for each of the points in the unit square
		...
		// NOTE: a vertex is "done" when it is connected to all other vertices, or when any new line would cross an existing line
		boolean graphComplete = false;
		
		while (!graphComplete) {
		
		//generates a random number corresponding to the vertex id 
		int randomVertexId = rand.nextInt(size + 1);
			
		// Connect X to nearest point Y (s.t. X is not already connected to Y and line crosses no other line)
		Vertex x = vertices.get(randomVertexId);
		double minDistance = Double.MAX_VALUE;
		int closestVertex = Integer.MAX_VALUE;
		
		// Moves through each of the vertices, calculating distance
		for (Vertex v: vertices) {
			if (v.getId() != randomVertexId) {
				// calculate distance
				distance = Math.sqrt(Math.pow((vertices.get(randomVertexId).getXLoc() - v.getXLoc()), 2) + Math.pow((vertices.get(randomVertexId).getYLoc() - v.getYLoc()), 2));
				
				// if this distance is less than minDistane, set new minDistance
				if (distance < minDistance) {
					minDistance = distance;
					closestVertex = v.getId();
					v.addNeighbors(closestVertex);
				} // end if	
			} // end if			
		} //end for - set distance back to maxval
		
		distance = Double.MAX_VALUE;
		minDistance = Double.MAX_VALUE;
		closestVertex = Integer.MAX_VALUE;
		
		// check if all vertices "done"
